...
internal static class Program
    {
        ...
        {
            int threads, rows, cols, ops, sleepTime;
            if (args.Length == 5)
            {
                rows = Int32.Parse(args[0]);
                cols = Int32.Parse(args[1]);
                threads = Int32.Parse(args[2]);
                ops = Int32.Parse(args[3]);
                sleepTime = Int32.Parse(args[4]);

                // create spreadsheet
                SharableSpreadSheet spreadsheet = new SharableSpreadSheet(rows, cols, threads);
                // fill the spreadsheet with strings
                ...
                List<Thread> threadsList = new List<Thread>(); // list of threads
                ...
                for (int i = 0; i < threads; i++)
                {
                    Thread thread = new Thread(() =>
                    {
                        ...
                        for (int j = 0; j < ops; j++)
                        {
                            ...
                            // get random string
                            int str = rnd.Next(1000);
                            int randomFunc = rnd.Next(15); // randomly select function from all functions
                            if (randomFunc == 1)
                            {
                                // set cell with random string
                                spreadsheet.setCell(row, col, "changed: " + str);
                                // print action result
                                Console.WriteLine("Thread " + Thread.CurrentThread.ManagedThreadId + " set cell " + row + "," + col + " to " + "changed: " + str);
                            }
                            else if (randomFunc == 2)
                            {
                                // get cell
                                String strCell = spreadsheet.getCell(row, col);
                                // print action result
                                Console.WriteLine("Thread " + Thread.CurrentThread.ManagedThreadId + " got cell " + row + "," + col + " String from cell: " + str);
                            }
                            else if (randomFunc == 3)
                            {
                                // find all
                                spreadsheet.findAll("testcell " + row + "," + col, true);
                                // print action result
                                Console.WriteLine("Thread " + Thread.CurrentThread.ManagedThreadId + " found all cells with " + "cell " + row + "," + col);
                            }
                            else if (randomFunc == 4)
                            {
                                // set all
                                spreadsheet.setAll("testcell " + row + "," + col, "changed: " + str, true);
                                // print action result
                                Console.WriteLine("Thread " + Thread.CurrentThread.ManagedThreadId + " set cell " + row + "," + col + " to " + "changed: " + str);
                            }
                            else if (randomFunc == 5)
                            {
                                // get size - print action result
                                Console.WriteLine("Thread " + Thread.CurrentThread.ManagedThreadId + " got size of spreadsheet: " + spreadsheet.getSize().ToString());
                            }
                            else if (randomFunc == 6)
                            {
                                // save spreadsheet to file
                                spreadsheet.save("spreadsheet.csv");
                                // print action result
                                Console.WriteLine("Thread " + Thread.CurrentThread.ManagedThreadId + " saved spreadsheet to file");
                            }
                            else if (randomFunc == 7)
                            {
                                // load spreadsheet from file
                                spreadsheet.load("spreadsheet.csv");
                                // print action result
                                Console.WriteLine("Thread " + Thread.CurrentThread.ManagedThreadId + " loaded spreadsheet from file");
                            }
                            else if (randomFunc == 8)
                            {
                                // exchange rows
                                int rowEx = rnd.Next(rows - 1);
                                spreadsheet.exchangeRows(row, rowEx);
                                // print action result
                                Console.WriteLine("Thread " + Thread.CurrentThread.ManagedThreadId + " exchange rows " + row + " and " + rowEx);
                            }
                            else if (randomFunc == 9)
                            {
                                // exchange cols
                                int colEx = rnd.Next(cols - 1);
                                spreadsheet.exchangeCols(col, colEx);
                                // print action result
                                Console.WriteLine("Thread " + Thread.CurrentThread.ManagedThreadId + " exchange columns " + col + " and " + colEx);
                            }
                            else if (randomFunc == 10)
                            {
                                // search in col
                                spreadsheet.searchInCol(col, "testcell " + row + "," + col);
                                // print action result
                                Console.WriteLine("Thread " + Thread.CurrentThread.ManagedThreadId + " search in column " + col + " for: " + "testcell " + row + "," + col);
                            }
                            else if (randomFunc == 11)
                            {
                                // search in row
                                spreadsheet.searchInRow(row, "testcell " + row + "," + col);
                                // print action result
                                Console.WriteLine("Thread " + Thread.CurrentThread.ManagedThreadId + " search in row " + row + " for: " + "testcell " + row + "," + col);
                            }
                            else if (randomFunc == 12)
                            {
                                // search in range
                                int rowEx = rnd.Next(row);
                                int colEx = rnd.Next(col);
                                spreadsheet.searchInRange(colEx, col, rowEx, row, "testcell " + row + "," + col);
                                Console.WriteLine("Thread " + Thread.CurrentThread.ManagedThreadId + " searched for " + "cell " + row + "," + col + " " + str + " in range: " + rowEx + "," + colEx + " to " + row + "," + col);
                            }
                            else if (randomFunc == 13)
                            {
                                // search string
                                spreadsheet.searchString("testcell " + row + "," + col);
                                // print action result
                                Console.WriteLine("Thread " + Thread.CurrentThread.ManagedThreadId + " searched for " + "testcell " + row + "," + col);
                            }
                            else if (randomFunc == 14)
                            {
                                // add row
                                spreadsheet.addRow(row);
                                // print action result
                                Console.WriteLine("Thread " + Thread.CurrentThread.ManagedThreadId + " added row after row " + row);
                            }
                            else if (randomFunc == 15)
                            {
                                // add col
                                spreadsheet.addCol(col);
                                // print action result
                                Console.WriteLine("Thread " + Thread.CurrentThread.ManagedThreadId + " added column after column " + col);
                            }
                            // sleep
